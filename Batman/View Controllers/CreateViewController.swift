...
final class CreateViewController: UIViewController {
    ...
    override func loadView() {
        ...
        let willShowObserver = center.addObserver(forName: .UIKeyboardWillShow, object: nil, queue: .main) { note in
            ...
            UIView.animate(withDuration: infos.1, delay: 0, options: infos.2, animations: {
                ...
                self.taskContent.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: infos.0, right: 0)
                self.view.layoutIfNeeded()
            }, completion: nil)
        }
        observersBag.append(willShowObserver)

        let willHideObserver = center.addObserver(forName: .UIKeyboardWillHide, object: nil, queue: .main) { note in
            guard let infos = parse(keyboardNotification: note) else { return }
        ...
        observersBag.append(willHideObserver)
    }

    deinit {
        observersBag.forEach { NotificationCenter.default.removeObserver($0) }
    }
